...
void huffman::create_preQueue(string text)
{
	int i = 0;
	while (i<text.size())
	{
...
{
	priority_queue<node_ptr, vector<node_ptr>, compare> temp(preQueue);
	while (temp.size() > 1)
	{
...
void huffman::encodeAndSave(string text) {
	create_preQueue(text);
	create_huffman_tree();
